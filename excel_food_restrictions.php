<?php
  ...
  $headerText = "Dog Days Ale " . date("Y") . " - Food Restrictions";
  $footerText = "";
  ...
  $sheet1 =& $xls->addWorksheet('Food Restrictions');
  ...
  $sheet1->write(0, 3, 'FOOD RESTRICTIONS', $titleFormat);
  ...
  while ( $row = mysql_fetch_array($result) ) 
  {
    ...
    $sheet1->write($currentRow, 3, $food_restrictions,  $rowFormat);

    $currentRow++;
  }
  
  // DONE! - output spreadsheet
  $xls_name = "food_restrictions_" . date("Y-m-d") . ".xls";
  $xls->send("$xls_name");
  $xls->close();
